...
async function load(id){render(await get('/api/runs/'+id))} function selectedAction(){let x=document.querySelector('#action').selectedOptions[0];document.querySelector('#action-category').textContent=x.dataset.category;return {action:x.value,category:x.dataset.category}} async function startRun(){busy('start-button',true,'Start monitoring');try{let name=document.querySelector('#scenario').value.trim()||'ui_security_run';let x=await post('/api/monitor/start',{scenario:name});document.querySelector('#control').textContent='Monitoring '+x.run_id;refreshState();}catch(e){alert(e)}finally{busy('start-button',false,'Start monitoring')}} async function executeAction(){busy('action-button',true,'Send action');try{let a=selectedAction();let x=await post('/api/action',a);document.querySelector('#control').textContent=x.ok?'Action completed and evidence refreshed.':'Action failed: '+x.detail;current=null;await refresh();refreshState();}catch(e){alert(e)}finally{busy('action-button',false,'Send action')}} async function stopRun(){busy('stop-button',true,'Finish run');try{let x=await post('/api/monitor/stop');document.querySelector('#control').textContent='Evidence downloaded for '+x.run_id;current=null;await refresh();refreshState();}catch(e){alert(e)}finally{busy('stop-button',false,'Finish run')}} document.querySelector('#action').addEventListener('change',selectedAction);selectedAction();refresh();refreshState();setInterval(refresh,3000);setInterval(refreshState,1500);
</script></body></html>"""
