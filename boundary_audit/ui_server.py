...
<section class="panel"><div class="panel-head"><div><div class="eyebrow">Network boundary</div><h2>External communication</h2></div><div class="help">Unattributed or inbound flows deserve investigation. “Allowed” only describes the observed verdict; it does not mean expected.</div></div><div class="row"><label for="ip-filter" class="muted">Hide IPs</label><input id="ip-filter" size="48" placeholder="Comma-separated IP addresses"><button onclick="applyIpFilter()">Apply filter</button><span class="muted">The laptop address and loopback are pre-filled.</span></div><table><thead><tr><th>Direction</th><th>Destination</th><th>Port</th><th>Protocol</th><th>Action link</th><th>Bytes out</th><th>Verdict</th></tr></thead><tbody id="flows"></tbody></table></section>
...
let current=null,autoSelect=true,defaultIpFilter=[];
function statusClass(s){return ['collected'].includes(s)?'good':['not_observed','partial'].includes(s)?'warn':'danger'}
function hiddenIps(){return new Set(document.querySelector('#ip-filter').value.split(',').map(x=>x.trim()).filter(Boolean))}
function applyIpFilter(){if(current)render(current)}
...
async function refreshState(){try{let s=await get('/api/state');if(!defaultIpFilter.length){defaultIpFilter=s.default_ip_filter||[];document.querySelector('#ip-filter').value=defaultIpFilter.join(', ')}document.querySelector('#state').textContent=(s.robot?.ok?'Robot connected':'Robot unavailable')+' · '+(s.monitoring?'Monitoring active':'Monitoring idle');}catch(e){document.querySelector('#state').textContent='Controller unavailable';}}
...
async function load(id){autoSelect=true;render(await get('/api/runs/'+id))} function selectedAction(){let x=document.querySelector('#action').selectedOptions[0];document.querySelector('#action-category').textContent=x.dataset.category;return {action:x.value,category:x.dataset.category}} async function startRun(){busy('start-button',true,'Start monitoring');try{let name=document.querySelector('#scenario').value.trim()||'ui_security_run';let x=await post('/api/monitor/start',{scenario:name});autoSelect=true;document.querySelector('#control').textContent='Monitoring '+x.run_id;refreshState();}catch(e){alert(e)}finally{busy('start-button',false,'Start monitoring')}} async function executeAction(){busy('action-button',true,'Send action');try{let a=selectedAction();let x=await post('/api/action',a);document.querySelector('#control').textContent=x.ok?'Action completed and evidence refreshed.':'Action failed: '+x.detail;autoSelect=true;current=null;await refresh();refreshState();}catch(e){alert(e)}finally{busy('action-button',false,'Send action')}} async function stopRun(){busy('stop-button',true,'Finish run');try{let x=await post('/api/monitor/stop');document.querySelector('#control').textContent='Run finalized and cleared. Evidence saved as '+x.run_id;autoSelect=false;clearView();await refresh();refreshState();}catch(e){alert(e)}finally{busy('stop-button',false,'Finish run')}} document.querySelector('#action').addEventListener('change',selectedAction);selectedAction();refresh();refreshState();setInterval(refresh,3000);setInterval(refreshState,1500);
</script></body></html>"""
