...
let current=null,autoSelect=true,defaultIpFilter=[],filterIps=[],connected=false;
...
async function refreshState(){try{let s=await get('/api/state');let connected=Boolean(s.robot?.ok);setConnected(connected);if(!defaultIpFilter.length){defaultIpFilter=s.default_ip_filter||[];filterIps=[...defaultIpFilter];renderIpTags();await refresh()}document.querySelector('#state').textContent=(connected?'Robot connected':'Robot unavailable')+' · '+(s.monitoring?'Monitoring active':'Monitoring idle');}catch(e){setConnected(false);document.querySelector('#state').textContent='Controller unavailable';}}
