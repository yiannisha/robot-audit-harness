...
def _default_ip_filter(extra: Optional[str] = None) -> List[str]:
    """Return addresses generated by the controller host, plus loopback."""
    addresses = {"127.0.0.0", "127.0.0.1"}
    if extra:
        try:
            for result in socket.getaddrinfo(extra, None):
                address = result[4][0]
                if ":" not in address:
                    addresses.add(address)
        except OSError:
            if all(char.isdigit() or char == "." for char in extra):
                addresses.add(extra)
    try:
    ...
    def state(self) -> Dict[str, Any]:
        result: Dict[str, Any] = {"monitoring": self.monitoring, "active_run_id": self.active_run_id,
                                  "default_ip_filter": _default_ip_filter(
                                      self.robot_target.rsplit(":", 1)[0] if self.robot_target else None)}
        if self.robot:
            try:
...
let current=null,autoSelect=true,defaultIpFilter=[],filterIps=[],connected=false,startingMonitoring=false;
...
function setConnected(value){connected=value;document.querySelector('#action').disabled=!connected;document.querySelector('#action-button').disabled=!connected;document.querySelector('#stop-button').disabled=!connected}
async function refreshState(){try{let s=await get('/api/state');let robotConnected=Boolean(s.robot?.ok);if(!defaultIpFilter.length){defaultIpFilter=s.default_ip_filter||[];filterIps=[...defaultIpFilter];renderIpTags();await refresh()}if(robotConnected&&!s.monitoring&&!startingMonitoring){startingMonitoring=true;await startRun();startingMonitoring=false;s.monitoring=true}let ready=robotConnected&&Boolean(s.monitoring);setConnected(ready);document.querySelector('#state').textContent=(robotConnected?'Robot connected':'Robot unavailable')+' · '+(s.monitoring?'Monitoring active':'Monitoring inactive');}catch(e){startingMonitoring=false;setConnected(false);document.querySelector('#state').textContent='Controller unavailable';}}
function busy(id,on,label){let b=document.querySelector('#'+id);b.disabled=on||!connected;b.innerHTML=on?'<span class="spinner"></span>Working…':label}
async function load(id){autoSelect=true;render(await get('/api/runs/'+id))} function selectedAction(){let x=document.querySelector('#action').selectedOptions[0];document.querySelector('#action-category').textContent=x.dataset.category;return {action:x.value,category:x.dataset.category}} async function startRun(){try{let x=await post('/api/monitor/start',{scenario:'ui_security_run'});autoSelect=true;document.querySelector('#control').textContent='Monitoring '+x.run_id}catch(e){document.querySelector('#control').textContent='Automatic monitoring start failed: '+e.message}} async function ensureMonitoring(){try{let s=await get('/api/state');if(s.robot?.ok&&!s.monitoring)await startRun()}catch(e){document.querySelector('#control').textContent='Automatic monitoring start failed: '+e.message}} async function executeAction(){busy('action-button',true,'Send action');try{let a=selectedAction();let x=await post('/api/action',a);document.querySelector('#control').textContent=x.ok?'Action completed and evidence refreshed.':'Action failed: '+x.detail;autoSelect=true;current=null;await refresh();refreshState();}catch(e){alert(e)}finally{busy('action-button',false,'Send action')}} async function stopRun(){busy('stop-button',true,'Clear run');try{let x=await post('/api/monitor/stop');document.querySelector('#control').textContent='Run finalized and cleared. Evidence saved as '+x.run_id;autoSelect=false;clearView();await refresh();refreshState();}catch(e){alert(e)}finally{busy('stop-button',false,'Clear run')}} document.querySelector('#action').addEventListener('change',selectedAction);document.querySelector('#ip-entry').addEventListener('keydown',event=>{if(event.key==='Enter'||event.key===','){event.preventDefault();applyIpFilter()}});selectedAction();(async()=>{await refresh();await refreshState();await ensureMonitoring();await refresh();await refreshState()})();setInterval(refresh,3000);setInterval(refreshState,1500);
</script></body></html>"""
