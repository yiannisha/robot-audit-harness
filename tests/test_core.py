...
from boundary_audit.simulator import SimulatedDevice


def test_scope_and_policy(tmp_path):
    run = run_experiment("camera", NetworkMode.OBSERVE, tmp_path, SimulatedDevice(), repeats=3)
...
def test_reports_are_standalone(tmp_path):
    run = run_experiment("diagnostics", NetworkMode.OBSERVE, tmp_path, SimulatedDevice(), repeats=3)
    write_reports(run)
    report = (run / "report.html").read_text()
    assert "no DNS evidence" in report
    assert "encrypted" in report.lower()


def test_enforce_records_blocked(tmp_path):
    run = run_experiment("camera", NetworkMode.ENFORCE, tmp_path, SimulatedDevice(), repeats=3)
...
def test_dashboard_contains_visualizations(tmp_path):
    run = run_experiment("camera", NetworkMode.OBSERVE, tmp_path, SimulatedDevice(), repeats=1)
    write_reports(run)
